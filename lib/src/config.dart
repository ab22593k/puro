...
class PuroInternalPrefsVars {
  ...
  static const _stringFields = {
    'defaultEnvironment',
    'lastUpdateCheck',
    'lastUpdateNotification',
    'profileOverride',
    'lastUpdateNotificationCommand',
    'pubCacheDir',
    'flutterGitUrl',
    'engineGitUrl',
    'dartSdkGitUrl',
  ...
  Future<dynamic> readVar(String key) async {
    if (!_validKeys.contains(key)) {
      throw 'No such key ${jsonEncode(key)}, valid keys: ${_validKeys.toList()}';
    }
    ...
    await updateGlobalPrefs(
      ...
      fn: (prefs) {
        ...
        if (value == 'null') {
          ...
          if (_stringFields.contains(key) && !value.startsWith('"')) {
            data[key] = value;
          } else {
